...
class HuffmanCoding:
    ...
    def visualize_tree(
        ...
        """
        Визуализирует дерево Хаффмана с помощью matplotlib.
        ...
        def draw_node(node, x, y, dx, level=0):
            if node is None:
                return
            ...
            # Отрисовка узла
            circle = plt.Circle((x, y), 0.3, color=color, ec='black', lw=1)
            ax.add_patch(circle)
            ...
            if node.left:
                ...
                child_x = x - dx
                child_y = y - 1
                ax.plot([x, child_x], [y-0.3, child_y+0.3], 'k-', lw=1)
                ax.text((x + child_x)/2, (y + child_y)/2, '0', 
            ...
            if node.right:
                ...
                child_y = y - 1
                ax.plot([x, child_x], [y-0.3, child_y+0.3], 'k-', lw=1)
                ax.text((x + child_x)/2, (y + child_y)/2, '1', 
                       ha='center', va='center', fontsize=8, color='red')
                draw_node(node.right, child_x, child_y, dx/2, level+1)

        # Начальная отрисовка
        draw_node(root, 0, 0, 3)
        ...
        ax.set_aspect('equal')
        ax.axis('off')
        ax.set_title('Дерево кодов Хаффмана', fontsize=14, fontweight='bold')
